...
| 表单 `submit`（GET） | `VC_LOCATION { method:'submit', httpMethod:'get' }`；父级 `VC_NAVIGATE` |
| 表单 `submit`（POST，urlencoded） | `VC_LOCATION` 含 `formBody`；父级 `VC_NAVIGATE { method:'POST', body }` |
| 表单 `submit`（POST，含文件） | `formFiles: true`；Chromo 仍报 `POST_FORM_UNSUPPORTED` |
...
**build `20260728-v24`+**：该行为由 `frameBustGuard` 控制（默认开）。测 AST（`jsfilter_frame_spoof`）时：关掉「吞破框 open(_top)」或发 `VC_DEBUG_OPTIONS { frameBustGuard: false }`，再对比破框 `VC_LOCATION` 是否减少；也可配合导航探针看列表。
...
| 验证 | `node scripts/test-jsfilter-frame.js`；测效果时先关 `frameBustGuard`（见上） |
...
| Hash 路由（`#/page` + `location.hash`） | ✅ build `20260728-v11`+ 同文档 hash 本地 `replaceState` + `VC_HISTORY` |
...
- Preview：JSON 格式化、文本（`VC_NETWORK_BODY_READ_LINES` 按需读行）；图片 base64（`VC_NETWORK_BODY_READ`，build `20260729-v29`+ 对二进制保留原始字节再 base64，修复此前 UTF-8 解码损坏图片的问题）；音视频等二进制占位不渲染
